# golf_app_patched.py
# Patched and optimized Streamlit app for GGGolf League 2026
# - Removes top-level destructive sheet overwrite
# - Makes sheet reads robust and cached
# - Avoids storing large DataFrames in session_state
# - Wraps rerun calls safely and reduces forced reruns
# - Adds admin one-click reset inside authenticated admin UI only
...
def get_gsheets_conn():
    try:
        ...
        # Surface a friendly message in the UI where appropriate
        raise
...
def _empty_master_df():
    ...
    # Set safe dtypes for numeric columns
    numeric_cols = [
        "Week",
        "Pars_Count",
        "Birdies_Count",
...
# --- Cached read helper (lean) ---
@st.cache_data(ttl=10)
def _read_sheet_cached():
    """
    Read the sheet and return a DataFrame with canonical columns.
    If the sheet is empty or missing expected columns, return an empty master df.
    """
    conn = get_gsheets_conn()
    # Use the connection's read method appropriate to your connector
    # Many connectors expose .read() or .get_as_df(); adjust if needed.
    data = conn.read()
    if data is None or data.empty or "Player" not in data.columns:
        return _empty_master_df()
    # Keep only expected columns and ensure all MASTER_COLUMNS exist
    df = data.copy()
...
# --- Robust load_data wrapper with fallback ---
def load_data():
    """
    Try to read the main sheet. If the sheet is empty, missing, or reading fails,
    return a safe empty master dataframe so the app can continue.
    """
    try:
        df = _read_sheet_cached()
        # Store a small preview for fallback (avoid storing full df)
        ...
        for wait in backoff:
            ...
            try:
                ...
                continue
        # If all retries fail, return empty master and show a friendly warning
        try:
...
# --- Handicap calculation (unchanged logic, defensive) ---
...
# --- Save weekly data (safe write, minimal UI disruption) ---
def save_weekly_data(week, player, pars, birdies, eagles, score_val, hcp_val, pin):
    """
    Efficient save:
    - Read current sheet (cached)
    - Build new entry and only write if it changes the sheet
    - Update a small in-memory preview after successful write
    """
    ...
    try:
        if not existing_norm.reset_index(drop=True).equals(updated_df.reset_index(drop=True)):
            ...
            for i in range(attempts):
                try:
                    ...
                    # update small in-memory preview
        ...
        # Clear only cached read so next read pulls fresh data
        try:
            st.cache_data.clear()
        except Exception:
            pass

    # Set a flag and attempt a safe rerun
...
with tabs[0]:
    if not EXISTING_PLAYERS:
        ...
        # Check session validity (2-hour timeout)
        is_unlocked = (
...
# -------------------------
# Tab 3: GGG Challenge (streamlined)
# -------------------------
with tabs[3]:
    st.header("🏁 GGG Challenge")
    st.write(
        "Join our season challenge to earn prizes! Every dollar raised goes right back into supporting the league and improving the GGGolf experience for everyone."
    )
    st.divider()
    ...
    with col_main:
        ...
        st.markdown(
            "**Overview:** Use the GGG sleeve during league play. Return at least one ball from the sleeve at the season finale to qualify for the top prize."
        )
        st.divider()
...
with tabs[4]:
                    ...
                    try:
                        if sel_week == 1:
                            if not pre_season.empty:
                                ...
                                st.write("Week 1 handicap (example):", round((sum(scores) / len(scores)) - 36, 1))
                            else:
    ...
    # Additional info categories (Rules, Schedule, Prizes, Expenses, Members) can be implemented similarly
    # For brevity, they are omitted here but should follow the same defensive patterns.
...
with tabs[5]:
    ...
    # Placeholder registration form (implement your existing registration logic here)
                ...
                if submit_test:
                    if not t_name or not t_pin:
                        ...
                        try:
                            ...
                            # Clear caches and attempt safe rerun
                            try:
                                st.cache_data.clear()
                            except Exception:
                                pass
...
with tabs[6]:
    ...
    if not st.session_state.get("authenticated"):
        ...
        # --- One-Click Sheet Reset (Admin Only) ---
        ...
        confirm_reset = st.checkbox(
            "I understand this will permanently erase all league data in the sheet", key="confirm_reset_checkbox"
        )
        ...
        if st.button("Reset Google Sheet Now", disabled=not confirm_reset):
            try:
                ...
                try:
                    ...
                    conn = get_gsheets_conn()
                    # Optional: write an audit row to a separate sheet before overwriting (recommended)
                    # Example (pseudo): conn.append_to_sheet("audit", {"action":"reset","by":"admin","ts":time.time()})
        ...
        st.divider()
        # Danger zone with guarded reset (form-based) for additional destructive actions
        with st.expander("Danger Zone (other destructive actions)", expanded=False):
            ...
            if submit_reset:
                if confirm_text == "RESET" and confirm_pass == ADMIN_PASSWORD:
                    try:
                        # Placeholder for actual reset logic (e.g., clearing leaderboard only)
                        # Implement the specific reset logic here
...
# -------------------------
# Debug / small utilities (admin-only)
# -------------------------
if st.session_state.get("authenticated"):
